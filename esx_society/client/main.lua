...
function OpenBossMenu(society, close, options)
	...
	table.insert(elements, {label = "Bilancio", value = "balance"})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
		...
		elseif data.current.value == 'manage_employees' then
			if isBoss == 'boss' then
				...
				OpenManageEmployeesMenu(society, 'vice')
			end
		elseif data.current.value == 'manage_grades' then
			OpenManageGradesMenu(society)
		elseif data.current.value == 'balance' then
			ESX.TriggerServerCallback('esx_society:getSocietyMoney', function(money)
				ESX.ShowNotification('~g~BILANCIO SOCIETÀ~s~: $' .. money)
			end, society)	
		end
	end, function(data, menu)
		if close then
...
function OpenBossMenu2(society, close, options)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
		...
		elseif data.current.value == 'balance' then
			ESX.TriggerServerCallback('esx_society:getSocietyMoney', function(money)
				ESX.ShowNotification('~g~BILANCIO FAZIONE~s~: $' .. money)
			end, society)	
		end
...
function OpenManageEmployeesMenu(society, grade)
	local elements = {
...
function OpenRecruitMenu(society)
	...
	ESX.TriggerServerCallback('esx_society:getOnlzhjhoihyyqzctzgdglphckdpskldupacinePlayers', function(players)
...
function OpenRecruitMenu2(society)
	...
	ESX.TriggerServerCallback('esx_society:getOnlzhjhoihyyqzctzgdglphckdpskldupacinePlayers2', function(players)
